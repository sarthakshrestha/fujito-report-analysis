...
if df is None:
    ...
    if 'Remaining Balance to Collect' in df.columns and 'Customer Name' in df.columns:
        ...
        with st.expander("Customers with the Most Remaining Balance", expanded=True):
            ...
            remaining_balance = df[df['Remaining Balance to Collect'] > 0][['Agent','Customer Name', 'Remaining Balance to Collect']].sort_values(by='Remaining Balance to Collect', ascending=False).head(top_x)
